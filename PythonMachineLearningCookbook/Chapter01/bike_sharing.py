...
# 数据加载函数
def load_dataset(filename, upper):
	file_reader = csv.reader(open(filename, 'r'), delimiter=',')
	X, y = [], []
	for row in file_reader:
		X.append(row[2:upper])
		y.append(row[-1])
	# 提取特征名称
...
# 读取数据并打乱顺序并划分数据
X, y, feature_names = load_dataset("bike_day.csv", 13)
#X, y, feature_names = load_dataset("bike_hour.csv", 14)
...
rf_regressor = RandomForestRegressor(n_estimators=1000, #评估器（决策树）数量
	                                                     ...
	                                                     min_samples_split=2) #决策树分裂一个节点需要用到的最小数量样本
...
print(u"均方误差\t{0:.8f}".format(mse))
print(u"解释方差分\t{0:.8f}".format(evs))
